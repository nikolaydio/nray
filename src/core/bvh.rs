...
unsafe impl<T:Intersectable+Clone> Sync for BVH<T> {}
...
impl<T: Intersectable+Clone> BVH<T> {
    pub fn new(max_prims_node: u32, primitives : &mut Vec<T>) -> BVH<T> {
        ...
        let root = BVH::<T>::build(&mut working_set[..], 0, max_prims_node, 0);
            ...
            if bcent.max()[axis] == bcent.min()[axis] {
                if elems.len() < max_prims_node as usize {
                    ...
                    return Box::new(Node::Interior(bbox,
                        BVH::<T>::build(&mut elems[0..mid], offset, max_prims_node, depth + 1),
                        BVH::<T>::build(&mut elems[mid..], offset + mid, max_prims_node, depth + 1)))
                }
            }
    ...
    pub fn hit(primitives: &[T], node: &Node, ray: &Ray3<f32>) -> Option<GeomDiff> {
        match node {
            &Node::Interior(ref bbox, ref left, ref right) => {
            ...
            &Node::Leaf(ref bbox, start, count) => {
                if !intersect_ray_aabb(ray, bbox) {
                    return None;
                }
